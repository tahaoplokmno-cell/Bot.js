...
// أمر بدء البوت والمحفظة والترحيب بالزبائن
bot.start(async (ctx) => {
    const uId = String(ctx.chat.id); 
...
// الأزرار الرئيسية للبوت (نصوص الكيبورد السفلي)
...
// تعديل زر استرجاع الأموال ليتحول إلى طلب إدخال مبلغ
bot.hears('⚖️ استرجاع الأموال', ctx => {
    const uId = String(ctx.chat.id);
    userStates[uId] = { action: 'await_refund_amount' };
    ctx.reply("💰 **قسم طلب استرجاع الأموال:**\n━━━━━━━━━━━━━━━━━━━━\n✍️ يرجى كتابة المبلغ الذي تريد استرجاعه (بالدولار أو بالليرة السورية) ورقم حسابك لتحويل المستحقات:");
});

// استقبال الرسائل النصية والصور (الآدمن والزبائن)
bot.on(['text', 'photo'], async (ctx) => {
    ...
    if (state.action === 'await_new_rate') { 
        ...
        if(!isNaN(r)){ 
            db.exchange_rate = r; 
            saveDB(); 
            ctx.reply(`✅ تم تعديل الصرف إلى ${r} ل.س`); 
        } 
        userStates[uId] = { action: 'admin_dashboard' }; 
        return; 
    }
    ...
    if (state.action === 'await_ban_uid') { 
        ...
        db.banned[ctx.message.text] = true; 
        saveDB(); 
        ctx.reply("✅ تم حظر الحساب بنجاح."); 
        userStates[uId] = { action: 'admin_dashboard' }; 
        return; 
    }
    
    if (state.action === 'await_unban_uid') { 
        if(db.banned) delete db.banned[ctx.message.text]; 
        saveDB(); 
        ctx.reply("✅ تم إلغاء حظر الحساب."); 
        userStates[uId] = { action: 'admin_dashboard' }; 
        return; 
    }
    ...
    if (state.action === 'await_gift_amount') { 
        ...
        if(!isNaN(amt)){ 
            ...
            saveDB(); 
            ctx.reply("✅ تم إضافة الرصيد للمحفظة بنجاح يدوياً!"); 
            bot.telegram.sendMessage(state.targetUid, `🎉 تم إيداع رصيد هدية في حسابك بقيمة $${amt}`); 
        } 
        userStates[uId] = { action: 'admin_dashboard' }; 
        return; 
    }
    ...
    // استقبال قيمة طلب استرجاع الأموال وإرسالها للإدارة
    if (state.action === 'await_refund_amount' && ctx.message.text) {
        ...
        let refundMsg = `⚠️ **طلب استرجاع أموال جديد:**\n` +
                        ...
                        `📝 **المبلغ المطلوب وتفاصيل الحساب:**\n${ctx.message.text}`;
                        
        await ctx.telegram.sendMessage(config.ADMIN_CHANNEL_ID, refundMsg, { parse_mode: 'Markdown' }).catch(console.error);
        ctx.reply("🚀 تم إرسال طلب استرجاع الأموال الخاص بك إلى الإدارة بنجاح! سيتم مراجعة رصيدك وتحويل المستحقات لك قريباً.");
        userStates[uId] = null;
        return;
    }
    
    // إصلاح إرسال الكود الفوري مع التعليمات للزبون بمجرد كتابة الآدمن للكود
    if (state.action === 'admin_send_code_now' && ctx.message.text) { 
        await ctx.telegram.sendMessage(state.clientUId, 
...
// معالجة أزرار الكول باك (Inline Keyboards) للطلبات والتحكم
bot.on('callback_query', async (ctx) => {
    ...
    if (data === "confirm_order") {
        ...
        if (!state) return ctx.reply("❌ لا يوجد طلب نشط.");
        
        let uBal = db.users[uId]?.balance_usd || 0; 
        let rate = db.exchange_rate || 14500;
        
        let chanBtn = Markup.inlineKeyboard([
            [Markup.button.callback("✅ قبول وتفعيل الكود", `order_dec#accept#${uId}#${state.price}`)], 
            [Markup.button.callback("❌ رفض وإلغاء", `order_dec#reject#${uId}`)]
        ]);
        
        await ctx.telegram.sendMessage(config.ADMIN_CHANNEL_ID, 
`📥 **طلب شراء جديد:**\n━━━━━━━━━━━━━━━━━━━━\n👤 الزبون: ${ctx.from.first_name}\n🆔 آيدي التليجرام: \`${uId}\`\n💰 رصيده: *$${uBal.toFixed(2)}* (${(uBal * rate).toLocaleString()} ل.س)\n🎯 المنتج: *${state.item}*\n💵 السعر: *${state.price}$*\n🆔 آيدي اللعبة: \`${state.gameId}\``, 
        { reply_markup: chanBtn.reply_markup, parse_mode: 'Markdown' }).catch(console.error);
        
        ctx.reply("🚀 تم إرسال طلب الشراء الخاص بك بنجاح إلى الإدارة! انتظر موافقة المسؤول هنا ليظهر لك الكود."); 
        userStates[uId] = null; 
        return;
    }
