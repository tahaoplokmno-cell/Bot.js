async function handleStoreDecisions(ctx, bot, db, userStates, saveDB, uId) {
    ...
    if (data.startsWith("pay_approve#") || data.startsWith("pay_reject#")) {
        ...
        if (data.startsWith("pay_approve#")) {
            ...
            let finalUsd = isUsd ? val : (val / (db.exchange_rate || 14500));
            
            // تحديث الرصيد وحفظه فوراً في قاعدة البيانات لمنع ضياع الفلوس عند إعادة التشغيل
            db.users[cId].balance_usd = (db.users[cId].balance_usd || 0) + finalUsd; 
    ...
    if (data.startsWith("order_dec#")) {
        ...
        if (dec === "accept") {
            ...
            // خصم الرصيد وحفظ العملية فوراً
            db.users[cId].balance_usd -= pr; 
            if (typeof saveDB === 'function') saveDB(); 
