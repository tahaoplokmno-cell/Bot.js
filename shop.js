...
function handleShopCallback(ctx, data, uId, userStates, db) {
    ...
    if (data === "m#cards") {
        let buttons = [
            [Markup.button.callback("🎮 بطاقات ستيم", "shop_cat#بطاقات ستيم STEAM")],
            [Markup.button.callback("🎮 بطاقات إكس بوكس", "shop_cat#بطاقات إكس بوكس XBOX")],
            backToMain
        ];
    ...
    if (data.startsWith("buy_item#")) {
        ...
        return ctx.reply(`✍️ اكتب الآيدي (ID) الخاص بحسابك في *${catName}*:`, { 
            parse_mode: 'Markdown' 
        });
    }
}
